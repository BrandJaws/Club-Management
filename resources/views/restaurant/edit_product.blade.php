      ...
      <div class="col-md-12">
        <div class="portlet light ">
          <div class="portlet-body">
            <form action="{{route('restaurant.update_product',$product['id'])}}"  method="post" enctype="multipart/form-data">
              ...
              <div class="form-group {{($errors->has('restaurant_sub_category_id'))?'has-error':''}}">
                ...
                <select name="restaurant_sub_category_id" id="" class="form-control">
                  ...
                  <option value="{{$category['id']}}" {{(old('restaurant_sub_category_id') && old('restaurant_sub_category_id')==$category['id'])?'selected="selected"':($product['restaurant_sub_category_id'] == $category['id'])?'selected="selected"':'' }}>
              ...
              <div class="row">
                <div class="col-sm-4">
                  <div class="form-group  {{($errors->has('image'))?'has-error':''}}"> <img src="{{asset($product['image'])}}" alt="" class="img-thumbnail" />
              ...
              <div class="form-group {{($errors->has('in_stock'))?'has-error':''}}">
                ...
                <select name="in_stock" id="" class="form-control">
                  <option value="YES" {{(old('in_stock') && old('in_stock')=="YES")?'selected="selected"':($product['in_stock'] == "YES")?'selected="selected"':'' }}>
                  
              Yes
              
                  </option>
                  <option value="NO" {{(old('in_stock') && old('in_stock')=="NO")?'selected="selected"':($product['in_stock'] == "NO")?'selected="selected"':'' }} >
              ...
              <div class="form-group {{($errors->has('visible'))?'has-error':''}}">
                ...
                <select name="visible" id="" class="form-control">
                  <option value="YES" {{(old('visible') && old('visible')=="YES")?'selected="selected"':($product['visible'] == "YES")?'selected="selected"':'' }}>
                  
              Yes
              
                  </option>
                  <option value="NO" {{(old('visible') && old('visible')=="NO")?'selected="selected"':($product['visible'] == "NO")?'selected="selected"':'' }} >
              ...
              <div class="fom-group text-right m-t-30">
                <button class="btn btn-lg red btn-outline btn-circle"> <i class="fa fa-floppy-o"></i> &nbsp;Update Product </button>
