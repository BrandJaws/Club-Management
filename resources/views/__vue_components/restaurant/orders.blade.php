  ...
  <table class="table table-hover">
    ...
    <tbody v-else>
      <tr v-for="(order,orderIndex) in ordersList">
        <td><a :href="baseUrl+'/admin/restaurant/orders/'+order.id">@{{ order.member_name }}</a></td>
        <td>$ @{{ order.gross_total }}</td>
        <td>@{{ order.time }}</td>
        <td><button v-if="order.in_process == 'NO'" class="btn btn-sm btn-outline rounded b-primary text-primary" v-on:click = "markAsInProcess(order)">In Process</button>
          ...
          <button v-else class="btn btn-sm rounded rounded green disabled">Is Served</button></td>
      </tr>
    </tbody>
  </table>
</template>
