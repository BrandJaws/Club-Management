		...
		<div class="col-md-12">
			...
			<div class="portlet light ">
				<div class="portlet-title">
					<div class="actions">
						<a href="{{route('employee.create')}}" class="btn red btn-outline btn-circle"> Create Employee</a>
					</div>
				</div>
					...
					<div class="row">
						<div class="col-md-12">
							...
							<table class="table table-hover customPaddingTable">
								...
								<tbody>
								@if(isset($data)) @foreach((array)$data['data'] as $employee)
									<tr>
										<td>{{$employee['firstName']}} {{$employee['lastName']}}</td>
										<td>{{$employee['email']}}</td>
										<td>{{$employee['phone']}}</td>
										<td class="action-btns">
											{!! Form::open(array('route' => array('employee.delete', $employee['id']), 'method' => 'delete')) !!}
											<div class="btn-group btn-group-circle" style="">
												<a style="padding: 5px 16px; width: 85px" href="{{route('employee.edit',['id'=>$employee['id']])}}" class="btn btn-outline green btn-sm"> <i class="fa fa-pencil"></i>&nbsp;Edit</a>
								...
								@endforeach @else
									<tr>
										<td colspan="6" style="text-align: center">Employees are not available</td>
									</tr>
								@endif
								</tbody>
							</table>
