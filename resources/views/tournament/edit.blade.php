		...
		<div class="col-md-12">
			<div class="portlet light">
				<div class="portlet-body">
					<div class="row">
						<div class="col-md-12">
							{!! Form::open(['url' => route('tournament.update',$tournament["id"]), 'id'=>'form-tournament',
					'class'=>'tournament-form', 'method'=>'put', 'enctype'=>'multipart/form-data']) !!}
							...
							<div class="form-group {{($errors->has('name'))?'has-error':''}}">
								...
								{!! Form::text('name',Input::old('name') ? Input::old('name') : $tournament["name"] , ['class'=>'form-control','placeholder'=>'Tournament name'] ) !!}
								@if($errors->has('name')) <span class="help-block">{{$errors->first('name') }}</span> @endif
							</div>

							<div class="form-group {{($errors->has('description'))?'has-error':''}}">
								<label class="control-label">Description</label>
								<textarea type="text" name="description" class="form-control" placeholder='Description'>{{Input::old('description') ? Input::old('description') : $tournament["description"] }}</textarea>
							...
							<div class="form-group {{($errors->has('tournamentStartDate'))?'has-error':''}}">
								...
								<div class="input-group date form_datetime form_datetime bs-datetime datepickerFormat" >
									<input type="text" size="16" class="form-control" name="tournamentStartDate"  placeholder="Start Date" value="{{Input::old('tournamentStartDate') ? Input::old('tournamentStartDate') : $tournament["tournamentStartDate"]  }}">
							...
							<div class="form-group {{($errors->has('numberOfWeeks'))?'has-error':''}}">
								...
								{!! Form::number('numberOfWeeks',Input::old('numberOfWeeks') ? Input::old('numberOfWeeks') : $tournament["numberOfWeeks"] , ['class'=>'form-control','placeholder'=>'Number of weeks'] ) !!}
							...
							<div class="form-group {{($errors->has('winning_criteria'))?'has-error':''}}">
								...
								<select class='form-control' name="winning_criteria">
									@foreach(config('global.tournament.winning_criteria') as $winningCriteria)
										<option value="{{$winningCriteria}}" {{Input::old('winning_criteria') &&  Input::old('winning_criteria') == $winningCriteria ? 'selected' : ($tournament["winning_criteria"] == $winningCriteria ? 'selected' : '') }}>{{$winningCriteria}}</option>
									@endforeach
								</select>
							...
							<div class="form-group {{($errors->has('pointsPerWin'))?'has-error':''}}">
								...
								{!! Form::number('pointsPerWin',Input::old('pointsPerWin') ? Input::old('pointsPerWin') : $tournament["pointsPerWin"], ['class'=>'form-control','placeholder'=>'Points Per Win'] ) !!}
							...
							<div class="form-group {{($errors->has('pointsPerLoss'))?'has-error':''}}">
								...
								{!! Form::number('pointsPerLoss',Input::old('pointsPerLoss') ? Input::old('pointsPerLoss') : $tournament["pointsPerLoss"], ['class'=>'form-control','placeholder'=>'Points Per Loss'] ) !!}
							...
							<fieldset border="1px solid">
								<div class="form-group {{($errors->has('registrationStartAt'))?'has-error':''}}">
									...
									<div class="input-group date form_datetime form_datetime bs-datetime datepickerFormat">
										<input type="text" name="registrationStartAt" size="16" class="form-control"  placeholder="Starts Date" value="{{Input::old('registrationStartAt') ? Input::old('registrationStartAt') : $tournament["registrationStartAt"] }}">
									<span class="input-group-addon">
										<button class="date-set" type="button">
											<i class="fa fa-calendar"></i>
										</button>
								...
								<div class="form-group {{($errors->has('registrationCloseAt'))?'has-error':''}}">
									...
									<div class="input-group date form_datetime form_datetime bs-datetime datepickerFormat">
										<input type="text" size="16" class="form-control" name="registrationCloseAt"  placeholder="End Date" value="{{Input::old('registrationCloseAt') ? Input::old('registrationCloseAt') : $tournament["registrationCloseAt"] }}">
									<span class="input-group-addon">
										<button class="date-set" type="button">
											<i class="fa fa-calendar"></i>
										</button>
							...
							<div class="form-group m-t-30 text-right">
								{!! Form::button('Update Tournament', ['class'=>'btn red btn-lg btn-outline btn-circle','type'=>'submit']) !!}
							</div>
...
<script>
	...
	var vue = new Vue({
		...
		data:{
			...
			events:{!! Input::old('tournament_events') && trim(Input::old('tournament_events')) != '[]' ? Input::old('tournament_events') : json_encode($tournament['tournament_events']) !!},
			schedulingDetails:{!! Input::old('scheduling_details') && trim(Input::old('scheduling_details')) != '[]' ? Input::old('scheduling_details') : $tournament['scheduling_details'] !!},
