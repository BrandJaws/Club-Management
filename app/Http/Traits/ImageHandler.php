...
trait ImageHandler {
	public static function uploadImage(UploadedFile $file, $path, $md5String, $upload = true, $clearOld = true) {
		try {
			$path = str_replace ( [ 
					'{member_id}',
					'{club_id}',
	...
	public static function getMultipartFileParams(\Illuminate\Http\Request $request, $keys = []) {
	    $files = [ ];
	    foreach ( ( array ) $keys as $key => $value ) {
	        if ($request->hasFile ( $value ) && $request->file ( $value )->isValid ()) {
	            $files [] = [
	                'name' => $value,
	                'contents' => \File::get ( $request->file ( $value )->getRealPath () ),
